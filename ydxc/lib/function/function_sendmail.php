...
/*
	...
	$Id: function_sendmail.php 2009-4-15 23:32:55 jerry $
*/

!defined('IN_CTB') && die('Access Denied');

function sendmail($toemail, $subject, $message, $other=array(), $from='') {
	global $_CTB, $_TCONFIG, $_TGLOBAL;
	
	$_TCONFIG['linkguide'] = 0;
	...
	//发信者
	if($mail['mailsend'] == 3) {
		$email_from = empty($from) ? $_TCONFIG['siteemail'] : $from;
	} else {
		$email_from = $from == '' ? '=?'.$_CTB['charset'].'?B?'.base64_encode($_TCONFIG['sitename'])."?= <".$_TCONFIG['siteemail'].">" : (preg_match('/^(.+?) \<(.+?)\>$/',$from, $mats) ? '=?'.$_CTB['charset'].'?B?'.base64_encode($mats[1])."?= <$mats[2]>" : $from);
	}
	
	$email_to = preg_match('/^(.+?) \<(.+?)\>$/',$toemail, $mats) ? ($mailusername ? '=?'.$_CTB['charset'].'?B?'.base64_encode($mats[1])."?= <$mats[2]>" : $mats[2]) : $toemail;;
	
	$email_subject = '=?'.$_CTB['charset'].'?B?'.base64_encode(preg_replace("/[\r|\n]/", '', '['.$_TCONFIG['sitename'].'] '.$subject)).'?=';
	$email_message = chunk_split(base64_encode(str_replace("\n", "\r\n", str_replace("\r", "\n", str_replace("\r\n", "\n", str_replace("\n\r", "\r", $message))))));
	
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: UCENTER_HOME ".X_VER."{$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=$_CTB[charset]{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	if($mail['mailsend'] == 1) {
		...
		if(substr($lastmessage, 0, 3) != 250) {
			fputs($fp, "RCPT TO: <".preg_replace("/.*\<(.+?)\>.*/", "\\1", $toemail).">\r\n");
			$lastmessage = fgets($fp, 512);
			runlog('SMTP', "($mail[server]:$mail[port]) RCPT TO - $toemail - $lastmessage", 0);
			return false;
		}
