...
if (isset($_POST['usage'])) {
	...
	//判断 父级 是否存在
	if($_POST['pid']){
		$query  = $_TGLOBAL['db']->query("SELECT NULL FROM ".tname('news_class')." WHERE id='$_POST[pid]' AND id<>$id");
	...
	if ($id <= 0) { //添加
		...
		inserttable('news_class', $classarr);
		makecache('news_class','id', '', 'orderlist', 1); //缓存
		showmsg('类别添加成功！', 'module.php?ac=news&mod=class&class_id='.$_POST['class_id']);
		...
		updatetable('news_class', $classarr, array('id' => $id));
		makecache('news_class','id', '', 'orderlist', 1); //缓存
		showmsg('类别修改成功！', 'module.php?ac=news&mod=class&class_id='.$_POST['class_id']);
...
if ($id > 0) {
	$tree = $_TGLOBAL['tree_news_class'][$id];
}
      ...
      <tbody>
        <tr>
          ...
          <td width="70%"><?php 
				...
				echo makeselect((array)$_TGLOBAL['tree_news_class'],'pid', $tree['pid'],'datatype="Require" msg="请选择父级菜单."','name', 'id','作为一级菜单', '0', 1) ;?></td>
        </tr>
        <tr>
