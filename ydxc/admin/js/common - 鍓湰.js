...
function fetchCheckbox(cbn) {
	return _$(cbn) && _$(cbn).checked == true ? 1 : 0;
}
...
function thumbImg(obj, method) {
	...
	if(!zw) {
		...
		setTimeout("thumbImg(_$('" + obj.id + "'), " + method + ")", 100);
		return;
	}
	zr = zw / zh;
	method = !method ? 0 : 1;
...
function dconfirm(msg, script, width, height) {
	...
	_$('floatwin_confirm_title').innerHTML = '提示信息';
	_$('floatwin_confirm_content').innerHTML = msg + '<br /><button onclick="' + script + ';floatwin(\'close_confirm\')">&nbsp;是&nbsp;</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button onclick="floatwin(\'close_confirm\')">&nbsp;否&nbsp;</button>';
}
...
function dnotice(msg, script, width, height) {
	...
	_$('floatwin_confirm_title').innerHTML = '提示信息';
	_$('floatwin_confirm_content').innerHTML = msg + (script ? '<br /><button onclick="' + script + ';floatwin(\'close_confirm\')">确定</button>' : '');
}
...
function sidebar_collapse(lang) {
	if(lang[0]) {
		...
		_$('wrap').className = _$('wrap').className == 'wrap with_side s_clear' ? 'wrap s_clear' : 'wrap with_side s_clear';
	} else {
		var collapsed = getcookie('discuz_collapse');
...
function display(id) {
	_$(id).style.display = _$(id).style.display == '' ? 'none' : '';
}
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn) {
	...
	ajaxpostHandle = [showid, ajaxframeid, formid, _$(formid).target, showidclass, submitbtn];
...
function ajaxpost_load() {
	...
	try {
		if(is_ie) {
			s = _$(ajaxpostHandle[1]).contentWindow.document.XMLDocument.text;
		} else {
			s = _$(ajaxpostHandle[1]).contentWindow.document.documentElement.firstChild.nodeValue;
		}
	} catch(e) {
		if(ajaxdebug) {
			var error = mb_cutstr(_$(ajaxpostHandle[1]).contentWindow.document.body.innerText.replace(/\r?\n/g, '\\n').replace(/"/g, '\\\"'), 200);
	...
	if(ajaxpostHandle[4]) {
		_$(ajaxpostHandle[0]).className = ajaxpostHandle[4];
		if(ajaxpostHandle[5]) {
			ajaxpostHandle[5].disabled = false;
		}
	}
	if(!evaled && (typeof ajaxerror == 'undefined' || !ajaxerror)) {
		ajaxinnerhtml(_$(ajaxpostHandle[0]), s);
		if(!evaled)evalscript(s);
		setMenuPosition(_$(ajaxpostHandle[0]).ctrlid, 0);
		setTimeout("hideMenu()", 3000);
	}
	ajaxerror = null;
	if(_$(ajaxpostHandle[2])) {
		_$(ajaxpostHandle[2]).target = ajaxpostHandle[3];
	}
	ajaxpostHandle = 0;
}
	...
	if(cache && div) {
		...
		x.get(href + '&inajax=1&ajaxmenuid='+ctrlid+'_menu', function(s) {
			...
			if(s.indexOf('ajaxerror') != -1) {
				evalscript(s);
				evaled = true;
				if(!cache && duration != 3 && x.div.id) setTimeout('_$("append_parent").removeChild(_$(\'' + x.div.id + '\'))', timeout);
			}
			if(!evaled && (typeof ajaxerror == 'undefined' || !ajaxerror)) {
...
function showloading(display, waiting) {
	...
	_$('ajaxwaitid').innerHTML = waiting;
...
function loadselect(id, showinput, pageobj, pos, method) {
	...
	for(var i = 0;i < obj.options.length;i++){
		...
		if(obj.options[i].selected) {
			...
			if(_$(objname)) {
				_$(objname).setAttribute('selecti', i);
			}
		}
	}
	lis += '</ul>';
...
function loadselect_key(ctrlobj, e, objname, pageobj) {
	...
	if(value == 40 || value == 38) {
		if(_$(objname + '_selectmenu').style.display == 'none') {
			loadselect_viewmenu(ctrlobj, objname, 0, pageobj);
		} else {
			lis = _$(objname + '_selectmenu').getElementsByTagName('LI');
			selecti = _$(objname).getAttribute('selecti');
			...
			_$(objname).setAttribute('selecti', selecti);
...
function loadselect_viewmenu(ctrlobj, objname, hidden, pageobj) {
	...
	if(_$(objname + '_selectmenu').style.display == '' || hidden) {
		_$(objname + '_selectmenu').style.display = 'none';
	} else {
		if(_$(selectopen)) {
			_$(selectopen).style.display = 'none';
		}
		var objoffset = fetchOffset(ctrlobj);
		if(pageobj) {
			var InFloate = pageobj.split('_');
			objoffset['left'] -= _$(pageobj).scrollLeft + parseInt(floatwinhandle[InFloate[1] + '_1']);
			objoffset['top'] -= _$(pageobj).scrollTop + parseInt(floatwinhandle[InFloate[1] + '_2']);
		}
		objoffset['height'] = ctrlobj.offsetHeight;
		_$(objname + '_selectmenu').style.display = '';
...
function loadselect_liset(objname, showinput, obj, v, opt, selecti) {
	var change = 1;
	if(showinput) {
		if(_$(objname + '_selectinput').value != opt) {
			_$(objname + '_selectinput').value = opt;
		} else {
			change = 0;
		}
	} else {
		if(_$(objname + '_selectinput').innerHTML != opt) {
			_$(objname + '_selectinput').innerHTML = opt;
		} else {
			change = 0;
		}
	}
	lis = _$(objname + '_selectmenu').getElementsByTagName('LI');
	lis[_$(objname).getAttribute('selecti')].className = '';
	lis[selecti].className = 'current';
	_$(objname).setAttribute('selecti', selecti);
	_$(objname + '_selectmenu').style.display='none';
	if(change) {
...
function showselect(obj, inpid, t, rettype) {
	if(!obj.id) {
		...
		if(!t) {
			...
			if(_$(t)) {
				var lis = _$(t).getElementsByTagName('LI');
				for(i = 0;i < lis.length;i++) {
					s += '<a href="javascript:;" onclick="_$(\'' + inpid + '\').value = this.innerHTML">' + lis[i].innerHTML + '</a><br />';
				}
				s += showselect_row(inpid, '自定义', -1);
			} else {
				s += '<a href="javascript:;" onclick="_$(\'' + inpid + '\').value = \'0\'">永久</a><br />';
...
function showselect_row(inpid, s, v, notime, rettype) {
	if(v >= 0) {
		if(!rettype) {
			...
			d = new Date();
			d.setTime(t);
			return '<a href="javascript:;" onclick="_$(\'' + inpid + '\').value = \'' + d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() + (!notime ? ' ' + d.getHours() + ':' + d.getMinutes() : '') + '\'">' + s + '</a><br />';
		} else {
			return '<a href="javascript:;" onclick="_$(\'' + inpid + '\').value = \'' + v + '\'">' + s + '</a><br />';
		}
	} else if(v == -1) {
		return '<a href="javascript:;" onclick="_$(\'' + inpid + '\').focus()">' + s + '</a><br />';
	} else if(v == -2) {
		return '<a href="javascript:;" onclick="_$(\'' + inpid + '\').onclick()">' + s + '</a><br />';
	}
}

//Smilies
function smilies_show(id, smcols, method, seditorkey) {
	if(seditorkey && !_$(seditorkey + 'smilies_menu')) {
		...
		_$(seditorkey + 'smilies_menu').appendChild(div);
	}
...
function smilies_onload(id, smcols, method, seditorkey) {
	...
	if(typeof smilies_type == 'object') {
		...
		for(i in smilies_type) {
			if(smilies_type[i][0]) {
				smiliestype += '<li><a href="javascript:;" hidefocus="true" ' + (currentstype == i ? 'class="current"' : '') + ' id="stype_'+method+'_'+i+'" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', '+i+', 1, ' + method + ', \'' + seditorkey + '\');if(currentstype) {_$(\'stype_'+method+'_\'+currentstype).className=\'\';}this.className=\'current\';currentstype='+i+';">'+smilies_type[i][0]+'</a></li>';
			}
		}
		smiliestype += '</ul></div>';
		_$(id).innerHTML = smiliestype + '<div style="clear: both" class="float_typeid" id="' + id + '_data"></div><table class="smilieslist_table" id="' + id + '_preview_table" style="display: none"><tr><td class="smilieslist_preview" id="' + id + '_preview"></td></tr></table><div style="clear: both" class="smilieslist_page" id="' + id + '_page"></div>';
...
function smilies_switch(id, smcols, type, page, method, seditorkey) {
	...
	_$(id + '_data').innerHTML = smiliesdata;
	_$(id + '_page').innerHTML = smiliespage;
}

function smilies_preview(id, obj, v) {
	if(!obj) {
		_$(id + '_preview_table').style.display = 'none';
	} else {
		_$(id + '_preview_table').style.display = '';
		_$(id + '_preview').innerHTML = '<img width="' + v + '" src="' + obj.childNodes[0].src + '" />';
	}
}
...
function seditor_insertunit(key, text, textend, moveend) {
	...
	if(!isUndefined(_$(key + 'message').selectionStart)) {
		var opn = _$(key + 'message').selectionStart + 0;
		if(textend != '') {
			text = text + _$(key + 'message').value.substring(_$(key + 'message').selectionStart, _$(key + 'message').selectionEnd) + textend;
		}
		_$(key + 'message').value = _$(key + 'message').value.substr(0, _$(key + 'message').selectionStart) + text + _$(key + 'message').value.substr(_$(key + 'message').selectionEnd);
		if(!moveend) {
			_$(key + 'message').selectionStart = opn + strlen(text) - endlen;
			_$(key + 'message').selectionEnd = opn + strlen(text) - endlen;
		}
...
function noticewin(s, t, c) {
	...
	_$('ntcwin').innerHTML = s;
	_$('ntcwin').style.display = '';
	_$('ntcwin').style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=0)';
	_$('ntcwin').style.opacity = 0;
	...
	_$('ntcwin').style.left = ((document.documentElement.clientWidth - _$('ntcwin').clientWidth) / 2) + 'px';
	_$('ntcwin').style.top = pbegin + 'px';
}

function noticewin_show(b, e, a, t) {
	step = (b - e) / 10;
	newp = (parseInt(_$('ntcwin').style.top) - step);
	if(newp > e) {
		_$('ntcwin').style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + a + ')';
		_$('ntcwin').style.opacity = a / 100;
		_$('ntcwin').style.top = newp + 'px';
		setTimeout(function () {noticewin_show(b, e, a += 10, t)}, 10);
	} else {
		_$('ntcwin').style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=100)';
		_$('ntcwin').style.opacity = 1;
...
function showimmestatus(imme) {
	...
	_$('imme_status_' + imme.id.substr(0, imme.id.indexOf('@'))).innerHTML = lang[imme.statusText];
}
...
function CheckAll(form,flag,prefix)
{
  var len = strlen(prefix);
  for (var i=0;i<form.elements.length;i++)
    {
    var e = form.elements[i];
    if (e.name.substring(0,len) == prefix)
       e.checked = flag;
    }
}
