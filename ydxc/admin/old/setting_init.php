...
if(isset ($_POST['attrsubmit'])) {
    ...
    $_TGLOBAL['db']->query("DELETE FROM ".tname('product_init')." WHERE id IN ($aryid) "); 
	showmsg('物资名录删除操作成功！');
}
...
<BODY onLoad="return write_nav('<?php echo $_TGLOBAL['menu']['main'][$ac]; ?> &raquo; <a href=\'module.php?ac=<?php echo $ac; ?>&mod=<?php echo $mod; ?>\' target=\'main\'><?php echo $_TGLOBAL['menu'][$ac][$mod]; ?></a> &raquo; ');">
  <DIV class=datacontainer2 style="WIDTH: 97%">
	  ...
	  <DIV class=header><a href="module.php?ac=setting&mod=init_add">添加初始化物资</a> </DIV>
	...
	<!--&nbsp;
		...
		echo makeselect((array)$_TGLOBAL['supplier'],'supplierid', $supplierid,'datatype="False" msg="请选择供应商."','name', 'id','选择供应商', '', 1) ;?>-->
    <!--&nbsp;
    ...
    <input name="button" type="submit" id="button" value="查询" class="button">
        ...
        <tr>
          ...
          <td class="caption"><div align="center"><?php echo GetOrder("采购时间", "getdate", $field, $order); ?></div></td>
...
<?php
	...
	$count = $_TGLOBAL['db']->result($_TGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('product_init')." sp
												JOIN ".tname('name')." n ON sp.nameid = n.id
												".$where), 0);	
	$query = $_TGLOBAL['db']->query("SELECT sp.*, n.name goods_name FROM ".tname('product_init')." sp
												JOIN ".tname('name')." n ON sp.nameid = n.id
...
?>
        <tr bgcolor="<?php echo $bgcolor = (isset($bgcolor) && $bgcolor == '#FFFFFF') ? '#F1F5FE' : '#FFFFFF'; ?>" onMouseOver="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='#E4EAF2';" onMouseOut="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='<?php echo $bgcolor; ?>';" onClick="this.bgColor=this.bgColor == '<?php echo $_TCONFIG['table_tr_click']; ?>' ? '<?php echo $bgcolor; ?>' : '<?php echo $_TCONFIG['table_tr_click']; ?>';">
		  ...
		  <td align="center"><?php echo $value['getdate'] ? date('Y-m-d', $value['getdate']) : 0; ?></td>
          ...
          <td align="center"><a href="module.php?ac=setting&mod=init_edit&classid=<?php echo $classid ;?>&id=<?php echo $value['id']; ?>">查看</a></td>
        </tr>
        <?php } ?>
      </table>
