...
if (isset($_POST['usage'])) {
	...
	if ($id <= 0) { //添加
		...
		inserttable('class_attr', $grouparr);
		//showmsg('物资属性添加成功！', 'module.php?ac=setting&mod=class_detail&classid='.$classid);
		showmsg('物资属性添加成功！', 'module.php?ac=setting&mod=class_attr&classid='.$classid);
		...
		updatetable('class_attr', $grouparr, array('id' => $id));
		//showmsg('物资属性修改成功！', 'module.php?ac=setting&mod=class_detail&classid='.$classid);
		showmsg('物资属性修改成功！', 'module.php?ac=setting&mod=class_attr&classid='.$classid);
...
if(!$classid){
	showmsg('请选择物资属性！', 'module.php?ac=setting&mod=class_detail');
}
if ($id > 0) {
	$query = $_TGLOBAL['db']->query("SELECT * FROM ".tname('class_attr')." WHERE id='$id'");
  ...
  <DIV class=datacontainer2 style="WIDTH: 97%">
    ...
    <form  method="post" name="FormEdit" onSubmit="return Validator.Validate(this,3)">
      <table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
        ...
        <tr>
          <td class="caption"><span class="font_red">*</span>属性名称</td>
        ...
        <tr>
          ...
          <tr>
            ...
            <input name="classid" type="hidden" id="id" value="<?php echo $classid; ?>"><!--物资属性编号-->
