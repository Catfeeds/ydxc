...
if(isset ($_POST['attrsubmit'])) {
	...
	//判断物资名录中有没有使用
    $query = $_TGLOBAL['db']->query("SELECT NULL FROM ".tname('name_attr')." WHERE attrsid IN($aryid)");									
    while ($value = $_TGLOBAL['db']->fetch_array($query)) {
        showmsg('物资名录中已经使用的物资属性不能删除！', $_TGLOBAL['refer']);
    } 
	
    $_TGLOBAL['db']->query("DELETE FROM ".tname('class_attr')." WHERE id IN ($aryid) "); 
	showmsg('物资属性删除操作成功！');
}
...
?>
  ...
  <ul>
    <li>如果物资名录中已经填写了这个物资属性，则不能删除。</li>
  </ul>
