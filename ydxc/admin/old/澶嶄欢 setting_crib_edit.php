...
if (isset($_POST['usage'])) {
	...
	if ($id <= 0) { //添加
		...
		// 更新编号、条形码
		$crib['no']      = str_repeat(0,3 - strlen($storeid)).$storeid;
		$crib['barcode'] = makeno('crib', 'barcode', 11, 'd'.$store['no']);;
		...
		showmsg('垛位添加成功！', 'module.php?ac=setting&mod=crib&id='.$_POST['id']);
		...
		makecache('crib','id', '', 'orderlist', 1); //缓存
		showmsg('垛位修改成功！', 'module.php?ac=setting&mod=crib&id='.$_POST['id']);
      ...
      <tbody>
        ...
        <tr>
          ...
          <td width="70%"><?php 
				...
				echo makeselect((array)$_TGLOBAL['tree_crib'],'pid', $tree['pid'],'datatype="Require" msg="请选择父级垛位."','name', 'id','作为一级垛位', '0', 1) ;?></td>
        </tr>
        <tr>
          <td width="30%" class="caption">垛位级别</td>
          <td width="70%"><?php 
				$tree['graded'] = isset($tree['graded']) ? $tree['graded'] : 0 ;
				echo makeselect($_TGLOBAL['setting']['crib_graded'],'graded', $tree['graded'],'datatype="Require" msg="请选择垛位级别."','', '','请选择垛位级别', '') ;?></td>
        </tr>
        <tr>
        ...
        <tr>
          <td width="30%" class="caption">所属仓库</td>
          <td width="70%"><input name="cribid" type="text" id="cribid" value="<?php echo isset($storename) ? $storename : '' ; ?>" size="20" maxlength="255" disabled /></td>
        </tr>
