...
?>
  ...
  <DIV class=datacontainer2 style="WIDTH: 98%">
   <!-- <DIV class=header><a href="module.php?ac=partner&mod=update" class="MustFill">添加新的活动</a></DIV>-->
      ...
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
        <tbody>
          <tr>
            ...
            <td class="caption"><div align="center"><?php echo GetOrder("姓名", "username", $field, $order); ?></div></td>
            <td class="caption"><div align="center"><?php echo GetOrder("电话", "phone", $field, $order); ?></div></td>
            <td class="caption"><div align="center"><?php echo GetOrder("学校", "school", $field, $order); ?></div></td>
            <!--<td class="caption"><div align="center"><?php echo GetOrder("活动", "actid", $field, $order); ?></div></td>-->
...
<?php
	...
	$where = " WHERE 1=1";
	$keywords && $where .= " AND (username LIKE '%$keywords%' OR phone = '$keywords')";
	$class_id && $where .= " AND class_id = '$class_id'";
	$count = $_TGLOBAL['db']->result($_TGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('partner_apply').$where), 0);
	$query = $_TGLOBAL['db']->query("SELECT * FROM ".tname('partner_apply')." $where ORDER BY $field $order LIMIT $start,$perpage");
...
?>
          <tr bgcolor="<?php echo $bgcolor = (isset($bgcolor) && $bgcolor == '#FFFFFF') ? '#F1F5FE' : '#FFFFFF'; ?>" onMouseOver="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='#E4EAF2';" onMouseOut="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='<?php echo $bgcolor; ?>';" onClick="this.bgColor=this.bgColor == '<?php echo $_TCONFIG['table_tr_click']; ?>' ? '<?php echo $bgcolor; ?>' : '<?php echo $_TCONFIG['table_tr_click']; ?>';">
            ...
            <td align="center"><?php echo $value['phone']; ?></td>
             ...
             <?php
//         		$sql = $_TGLOBAL['db']->query("SELECT * FROM ".tname('partner_rule')." where id=".$value['actid']);
//				$act = $_TGLOBAL['db']->fetch_array($sql);
           	?>
            <!--<td align="center"><?php echo $act['title']; ?></td>-->
             <td align="center"><?php echo date('Y-m-d',$value['inputtime']); ?></td>
           <!-- <td align="center"><a href="module.php?ac=partner&mod=update&id=<?php echo $value['id']; ?>">修改</a></td>-->
          </tr>
<?php } ?>
        </tbody>
      </table>
