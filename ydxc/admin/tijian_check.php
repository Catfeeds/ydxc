...
?>
  ...
  <DIV class=datacontainer2 style="WIDTH: 98%">
    ...
    <form  method="post" name="FormEdit">
      &nbsp;搜索
    <input name="keywords" type="text" id="keywords" size="30" placeholder="姓名，电话" value="<?php echo $keywords; ?>">
	&nbsp;
      ...
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
        <tbody>
          <tr>
            ...
            <td class="caption"><div align="center"><?php echo GetOrder("体检站", "station", $field, $order); ?></div></td>
            <td class="caption"><div align="center"><?php echo GetOrder("体检码", "old_code", $field, $order); ?></div></td>
            <td class="caption"><div align="center"><?php echo GetOrder("验证码", "code", $field, $order); ?></div></td>
            <td class="caption"><div align="center"><?php echo GetOrder("是否付款", "ispay", $field, $order); ?></div></td>
...
<?php 
	...
	$where = " WHERE ispay=2";
	$keywords && $where .= " AND (name LIKE '%$keywords%' OR phone LIKE '%$keywords%')";
	$class_id && $where .= " AND class_id = '$class_id'";
	$count = $_TGLOBAL['db']->result($_TGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('check').$where), 0);
	$query = $_TGLOBAL['db']->query("SELECT * FROM ".tname('check')." $where ORDER BY $field $order LIMIT $start,$perpage");
...
?>
          <tr bgcolor="<?php echo $bgcolor = (isset($bgcolor) && $bgcolor == '#FFFFFF') ? '#F1F5FE' : '#FFFFFF'; ?>" onMouseOver="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='#E4EAF2';" onMouseOut="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='<?php echo $bgcolor; ?>';" onClick="this.bgColor=this.bgColor == '<?php echo $_TCONFIG['table_tr_click']; ?>' ? '<?php echo $bgcolor; ?>' : '<?php echo $_TCONFIG['table_tr_click']; ?>';">
            ...
            <?php $sql = $_TGLOBAL['db']->query("SELECT * FROM ".tname('news')." where id=".$value['station']);
				$station = $_TGLOBAL['db']->fetch_array($sql); 	 ?>
            <td align="center"><?php echo $station['title']; ?></td>
            <td align="center"><?php echo $value['old_code']; ?></td>
            <td align="center"><?php echo $value['code']; ?></td>
            <td align="center"><?php if($value['ispay']==1){ ?>未付款<?php }elseif($value['ispay']==2){ ?>已付款<?php } ?></td>
            <td align="center"><?php echo date("Y-m-d H:i",$value['inputtime']); ?></td>
            <td align="center"><a href="module.php?ac=tijian&mod=update&id=<?php echo $value['id']; ?>">修改</a></td>
          </tr>
<?php } ?>
        </tbody>
      </table>
...
<?php
//echo @getclassselect('tj_code_info_class', '', 'onChange="this.form.dowhat[3].checked=true;"');
echo makeselect((array)$_TGLOBAL['tree_tj_code_info_class'],'class_id', $class_id,'datatype="Require" msg="请选择栏目"','name', 'id','请选择栏目', '0', 1);?> 
  ...
  <div align="center"><span class="page"><?php echo $multi; ?></span></div>
	...
	<script type="text/javascript">
		function dcqb(){
			window.location.href="/index.php?do=dc_tjcheck";
		}
		
		function dcby(){
			window.location.href="/index.php?do=dc_tjcheck&page=<?php echo $page ?>";
		}
	</script>
