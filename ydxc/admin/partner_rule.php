...
if (isset($_POST['btnsubmit'])) {
	...
	$aryid = simplode($_POST['itemid']);
	 ...
	 switch($_POST['dowhat']) {
		 ...
		 case 'delete' :
				//删除图片
				$query = $_TGLOBAL['db']->query("SELECT img FROM ".tname('partner_rule')." WHERE id IN($aryid)");
				while ($value = $_TGLOBAL['db']->fetch_array($query)) {
					file_exists('.'.$value['img']) && unlink('.'.$value['img']);
					file_exists('.'.$value['img'].'.thumb.jpg') && unlink('.'.$value['img'].'.thumb.jpg');
				}	 
				$_TGLOBAL['db']->query("DELETE FROM ".tname('partner_rule')." WHERE id IN($aryid)");
			 break;

		 case 'display' :
			  $_TGLOBAL['db']->query("UPDATE ".tname('partner_rule')." SET display=1 WHERE id IN($aryid)");
			  break;

		 case 'nodisplay' :
			 $_TGLOBAL['db']->query("UPDATE ".tname('partner_rule')." SET display=0 WHERE id IN($aryid)");
			 break;

		 case 'move' :
			$_TGLOBAL['db']->query("UPDATE ".tname('partner_rule')." SET class_id='$_POST[class_id]' WHERE id IN($aryid)");
			 break;
	}
	showmsg('内容操作成功！');
}
      ...
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
        <tbody>
          <tr>
            ...
            <td class="caption"><div align="center"><?php echo GetOrder("报名时间", "signup_time", $field, $order); ?></div></td>
...
<?php 
	...
	$count = $_TGLOBAL['db']->result($_TGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('partner_rule').$where), 0);
	$query = $_TGLOBAL['db']->query("SELECT * FROM ".tname('partner_rule')." $where ORDER BY $field $order LIMIT $start,$perpage");
...
?>
          <tr bgcolor="<?php echo $bgcolor = (isset($bgcolor) && $bgcolor == '#FFFFFF') ? '#F1F5FE' : '#FFFFFF'; ?>" onMouseOver="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='#E4EAF2';" onMouseOut="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='<?php echo $bgcolor; ?>';" onClick="this.bgColor=this.bgColor == '<?php echo $_TCONFIG['table_tr_click']; ?>' ? '<?php echo $bgcolor; ?>' : '<?php echo $_TCONFIG['table_tr_click']; ?>';">
            ...
            <td align="center"><?php echo date('Y-m-d',$value['signup_time']); ?></td>
