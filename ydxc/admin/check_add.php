...
if (isset($_POST['usage'])) {
	...
	if ($id <= 0) { //添加
		...
		$substring = substr($_POST['tj_code'],0,7);
		// 10位体检码  7位源码 ＋ 3位随机码
		$arr['tj_code_new'] = $substring.$rand_group;
		$id = inserttable('tj_code_info', $arr, 1);
		$result = '内容添加成功！';
    ...
    <table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
      <tbody>
        ...
        <tr>
          ...
          <td><input name="putouttime" type="date" id="putouttime" value="<?php echo date('Y-m-d',@$putouttime); ?>" size="85" maxlength="255" datatype="Require" msg="请填写发放时间." ></td>
        </tr>
        <tr>
          ...
          <td>
          	<select name="is_use">
          		...
          		<option value="1" <?php if(@$is_use==1){?>selected="selected"<?php } ?>>未使用</option>
          		<option value="2" <?php if(@$is_use==2){?>selected="selected"<?php } ?>>已使用</option>
          	</select>
          </td>
        </tr>
		<tr>
