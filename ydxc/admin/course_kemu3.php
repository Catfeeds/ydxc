      ...
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
        <tbody>
          <tr>
            ...
            <td class="caption"><div align="center"><?php echo GetOrder("选择教练", "coach_id", $field, $order); ?></div></td>
            ...
            <td class="caption"><div align="center"><?php echo GetOrder("选择教练时间", "k3_time", $field, $order); ?></div></td>
...
<?php
	...
	$where = " WHERE ispay=1 and coach3_id>0";
	$keywords && $where .= " AND (name LIKE '%$keywords%' OR sn='$keywords' OR mobile='$keywords')";
	$start_date_unx && $end_date_unx && $where .= " AND k3_time>='$start_date_unx' AND k3_time<='$end_date_unx'";
...
while ($value = $_TGLOBAL['db']->fetch_array($query)) {
	if(!empty($value['coach3_id'])){
	$coach = $_TGLOBAL['db']->getrow("SELECT * FROM ".tname('news')." WHERE id=".$value['coach3_id']);
...
?>
          <tr bgcolor="<?php echo $bgcolor = (isset($bgcolor) && $bgcolor == '#FFFFFF') ? '#F1F5FE' : '#FFFFFF'; ?>" onMouseOver="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='#E4EAF2';" onMouseOut="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='<?php echo $bgcolor; ?>';" onClick="this.bgColor=this.bgColor == '<?php echo $_TCONFIG['table_tr_click']; ?>' ? '<?php echo $bgcolor; ?>' : '<?php echo $_TCONFIG['table_tr_click']; ?>';">
            ...
            <td align="center"><?php echo date("Y-m-d H:i", $value['k3_time']); ?></td>
...
<script type="text/javascript">
	function dcqb(){
		window.location.href="/ydxc/index.php?do=dc_kemu3";
	}
	
	function dcby(){
		window.location.href="/ydxc/index.php?do=dc_kemu3&page=<?php echo $page ?>";
	}
	
	function dcrq(){
		window.location.href="/ydxc/index.php?do=dc_kemu3&start_date_unx=<?php echo $start_date_unx ?>&end_date_unx=<?php echo $end_date_unx ?>";
	}
</script>
