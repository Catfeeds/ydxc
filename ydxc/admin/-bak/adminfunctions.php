...
// ###################### 自定义函数，取得所有表的注释，并存于数组中 #######################
function ReadTableComment() {
	global $DB_site, $AryTableComment, $AryTableType;
	
	$result=$DB_site->query("SHOW TABLE STATUS");
	while ($currow=$DB_site->fetch_array($result)) {
	    $AryTableComment[$currow[Name]] = $currow[Comment];
		$AryTableType[$currow[Name]] = $currow[Type];
	}
...
// then reinitialises the bgcolor counter.
	global $bgcounter;
	echo "<tr class='tblhead'><td colspan='$colspan'><a name=\"$anchor\"><b><span class='tblhead'>".iif($htmlise,htmlspecialchars($title),$title)."</span></b></a></td></tr>";
	$bgcounter = 0;
}
...
// ###################### Start makeinputcode #######################
function makeinputcode ($title,$name,$value="",$htmlise=1,$size=35,$attr="") {
...
// second column contains an input box of name, $name and value, $value. $value is "HTMLised"
  ...
  echo "<tr class='".getrowbg()."' valign='top'>\n<td><p>$title</p></td>\n<td><p><input type=\"text\" size=\"$size\" name=\"$name\" value=\"$value\" $attr></p></td>\n</tr>\n";
}
...
// ###################### Start doformfooter #######################
function doformfooter($submitname="提交",$resetname="重选",$colspan=2,$goback="") {
...
// ###################### Start makenavoption #######################
// creates an <option> or <a href for the left-panel of index.html
...
// ###################### Start makenavselect #######################
// creates a <select> or <table> for the left panel of index.html
// (depending on value of $cpnavjs)
