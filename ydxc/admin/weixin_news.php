...
/*
	...
	$Id: weixin_weixin.php 2010-01-12 13:27:57 jerry $
*/
...
if(isset ($_POST['btnsubmit'])) {
	...
	$aryid = simplode($_POST['itemid']);
	 ...
	 switch($_POST['dowhat']) {
		 ...
		 case 'delete' :
				//删除图片
				$query = $_TGLOBAL['db']->query("SELECT img FROM ".tname('weixin')." WHERE id IN($aryid)");
		 ...
		 case 'move' :
			$_TGLOBAL['db']->query("UPDATE ".tname('weixin')." SET class_id='$_POST[class_id]' WHERE id IN($aryid)");
			 break;
	}
	showmsg('内容操作成功！');
}
...
//排序方式
$field  = isset($_GET['field']) ? $_GET['field'] : 'displayorder';
...
<BODY onLoad="return write_nav('<?php echo $_TGLOBAL['menu']['main'][$ac]; ?> &raquo; <?php echo $_TGLOBAL['menu'][$ac][$mod]; ?> &raquo; ');">
  <DIV class=datacontainer2 style="WIDTH: 98%">
    <DIV class=header><a href="module.php?ac=weixin&mod=news_add" class="MustFill">添加新的图文回复</a></DIV>
    <form  method="post" name="FormEdit">
      &nbsp;搜索
    <input name="keywords" type="text" id="keywords" size="30" value="<?php echo isset($keywords) ? $keywords : ''; ?>">
        类别查询
        <?php
echo @getclassselect('weixin_class', '', 'onChange="this.form.submit();"', $class_id);
?>
      ...
      <table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="tab_01">
        <tbody>
          <tr>
            ...
            <td class="caption"><div align="center"><?php echo GetOrder("标签", "flag", $field, $order); ?></div></td>
...
<?php 
	...
	$where = " WHERE itype='weixin'"; //只显示图文
...
?>
          <tr bgcolor="<?php echo $bgcolor = (isset($bgcolor) && $bgcolor == '#FFFFFF') ? '#F1F5FE' : '#FFFFFF'; ?>" onMouseOver="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='#E4EAF2';" onMouseOut="if (this.bgColor != '<?php echo $_TCONFIG['table_tr_click']; ?>') this.bgColor='<?php echo $bgcolor; ?>';" onClick="this.bgColor=this.bgColor == '<?php echo $_TCONFIG['table_tr_click']; ?>' ? '<?php echo $bgcolor; ?>' : '<?php echo $_TCONFIG['table_tr_click']; ?>';">
            ...
            <td align="left"><?php echo getclasscotename('weixin_class', $value['class_id']); ?></td>
            ...
            <td align="center"><?php echo $value['flag'] ? $_TGLOBAL['setting']['weixin_flag'][$value['flag']] : '&nbsp;'; ?></td>
            ...
            <td align="center"><a href="module.php?ac=weixin&mod=news_add&id=<?php echo $value['id']; ?>">修改</a></td>
          </tr>
<?php } ?>
        </tbody>
      </table>
...
<?php
echo getclassselect('weixin_class', '', 'onChange="this.form.dowhat[3].checked=true;"');
?>
