...
if (isset($_POST['usage'])) {
	...
	if ($id <= 0) { //添加
		...
		updatetable('check', $arr, array('id' => $id));
		$result = '内容修改成功！';
	}

	showmsg($result, 'module.php?ac=tijian&mod=check');
...
$id = @(int)$_GET['id'];
if ($id > 0) {
	$query = $_TGLOBAL['db']->query("SELECT * FROM ".tname('check')." WHERE id='$id'");
