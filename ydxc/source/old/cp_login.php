...
if(submitcheck('btnsubmit')  || ($op == 'weixinlog' && $openid) ) {
	...
	//判断此username是否存在
	if ($username) {
		$query = $_TGLOBAL['db']->query("SELECT id, password, times, isdisable FROM ".tname('members')." WHERE username='$username'");
	}
	if ($op == 'weixinlog' && $openid) {
		$query = $_TGLOBAL['db']->query("SELECT id, username, password, times, isdisable FROM ".tname('members')." WHERE openid='$openid'");
	}
	...
	if ($password != $userinfo['password']) {
		...
		showmessage_ajax(array('ok'=>'login_failure_please_re_login'));
	}

	if (1 == $userinfo['isdisable']) {
		//showmessage('users_were_close_please_re_login');
		showmessage_ajax(array('ok'=>'users_were_close_please_re_login'));
	}
	...
	updatetable('members', $userarr, array('uid' => $userinfo['id']));

	//设置cookie
	ssetcookie('auth', authcode("$password\t$userinfo[id]", 'ENCODE'), $_POST['islogged'] ? 2592000 : 0);
	ssetcookie('loginuser', 31536000);
	ssetcookie('_refer', '');

	$_TGLOBAL['ctb_uid'] = $userinfo[id];
	$_TGLOBAL['ctb_username'] = $username;
	//showmessage('login_success',$refer);
	showmessage_ajax(array('ok'=>'login_success', 'jumpto'=>$refer ));
