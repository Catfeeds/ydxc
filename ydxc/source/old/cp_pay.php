...
if ($_POST['manual']) {    
	...
	if ($payid) {		
		...
		//给用户推荐己支付模板
		template_enter($info['PlateNumber'], 'paied', $info['FeeMoney'], $payid, $info['ParkNumber']);
...
//查询车辆是否设置了自动缴费
$BindPay = getcount('member_car', "CarNumber='$info[PlateNumber]'",'BindPay');
if (!$BindPay) { //未绑定时，采用手工支付
    Header("Location: index.php?do=pay_manual&id=".$id);
}
